...
{
	magic_t const magic;   //magic 用于对当前 AutoreleasePoolPage 结构完整性的校验
	__unsafe_unretained id *next;  //next 指向了下一个为空的内存地址
	pthread_t const thread;  //thread 保存了当前页所在的线程
    
    //父子节把AutoreleasePoolPage以双向链表的形式连接起来
	AutoreleasePoolPage * const parent; //父节点,第一个结点的 parent 值为 nil
