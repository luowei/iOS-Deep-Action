...
objc_object::clearDeallocating()
{
    ...
    if (slowpath(!isa.nonpointer)) {
        ...
        //清理对象存储在SideTable中的引用计数数据
        sidetable_clearDeallocating();
    }
    //否则，判断是否有使用SideTable的辅助引用计数(isa.has_sidetable_rc)或者有weak引用(isa.weakly_referenced)
...
objc_object::rootDealloc()
{
    ...
    /*
     如果对象是采用了优化的isa计数方式，
     且同时满足对象没有被weak引用!isa.weakly_referenced、
     没有关联对象!isa.has_assoc 、
     没有自定义的C++析构方法!isa.has_cxx_dtor、
     没有用到SideTable来引用计数!isa.has_sidetable_rc则直接快速释放
     */
