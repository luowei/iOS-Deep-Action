...
//进程优先级,数值越大，优先级越高
/*
后台应用程序优先级JETSAM_PRIORITY_BACKGROUND是3，
低于前台应用程序优先级JETSAM_PRIORITY_FOREGROUND10，
而SpringBoard(桌面程序)位于JETSAM_PRIORITY_HOME16。
*/
...
//优先级进程列表
//内核里面对于所有的进程都有一个优先级的分布，通过一个数组维护，数组每一项是一个进程的list。
...
extern memstat_bucket_t memstat_bucket[MEMSTAT_BUCKET_COUNT];  //优先级队列，包含不同优先级的结构
