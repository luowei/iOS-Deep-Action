...
//初始化JETSAM线程，JetSAM 低内存事件的常驻监控线程的实现
...
memorystatus_init(void)
{
	...
	/*
	开启max_jetsam_threads个JetSam线程(性能差的设备为1个，其余为3个)，这些线程的优先级是内核所能分配的最高级(95, MAXPRI_KERNEL)。
	并且为每个线程增加了次序（注意:前文的-2~19是进程优先级区间，而这里的95是线程优先级，XNU的线程优先级范围是0~127）。
	*/
...
{
#if CONFIG_JETSAM
	//判断kill_under_pressure_cause值为kMemorystatusKilledVMThrashing，kMemorystatusKilledFCThrashing，kMemorystatusKilledZoneMapExhaustion时，
	//或者当前可用内存memorystatus_available_pages小于阈值memorystatus_available_pages_pressure时
	return is_reason_thrashing(kill_under_pressure_cause) ||
	       is_reason_zone_map_exhaustion(kill_under_pressure_cause) ||
	       memorystatus_available_pages <= memorystatus_available_pages_pressure;
	//kill_under_pressure_cause标记位,在调用 memorystatus_kill_process_async 异步杀进程时设置的，而后再唤醒OOM检测的
...
memorystatus_act_on_hiwat_processes(uint32_t *errors, uint32_t *hwm_kill, boolean_t *post_snapshot, __unused boolean_t *is_critical, uint64_t *memory_reclaimed)
{
	...
	*memory_reclaimed = 0;
	//这里直接调用memorystatus_kill_hiwat_proc，回收超限的进程
...
memorystatus_act_aggressive(uint32_t cause, os_reason_t jetsam_reason, int *jld_idle_kills, boolean_t *corpse_list_purged, boolean_t *post_snapshot, uint64_t *memory_reclaimed)
{
	...
	*memory_reclaimed = 0;
	...
	/*
	while 让这个函数不会只杀一个进程，若杀了一个进程后，发现内存还是会有压力，
	会继续杀进程，甚至会杀掉前台进程。
	*/
	...
	while (elevated_bucket_count) {
		 ...
		 */
		//如果内存压力依然存在，则通过memorystatus_kill_elevated_process杀掉后台进程
	 ...
	 */
	//如果杀掉了所有低优先级的进程，还有内存压力，再通过memorystatus_kill_top_process_aggressive杀掉优先级最低的进程
...
//OOM检测流程:  通过 memorystatus_thread 管理内存状态，回收内存，这是OOM管理线程的主函数
//管理内存状态，当内存状态出现问题或者内存压力过大时，将会通过一定的策略，干掉一些 App 回收内存。
static void
memorystatus_thread(void *param __unused, wait_result_t wr __unused)
{
	 ...
	 */
	//核心的代码在while (memorystatus_action_needed())循环里,memorystatus_action_needed()是触发OOM的核心判断条件
	while (memorystatus_action_needed()) {
		...
		//判断当前内存资源紧张的情况，即检测OOM
		cause = kill_under_pressure_cause;
		switch (cause) { //判断JetSAM出现的原因,设置原因状态码
		case kMemorystatusKilledFCThrashing:
		...
		/* Highwater */
		//当前内存紧张,调用memorystatus_act_on_hiwat_processes方法，触发high-water类型OOM
		...
		//如果上面通过high-water相关代码不能结束任何进程，将走到memorystatus_act_aggressive()函数，大部分OOM是在这里发生。
 ...
 */
//当物理内存达到限制时，触发回调，会调用memorystatus_on_ledger_footprint_exceeded，来同步触发per-process-limit类型的OOM。
void
memorystatus_on_ledger_footprint_exceeded(boolean_t warning, boolean_t memlimit_is_active, boolean_t memlimit_is_fatal)
{
	...
	if (memlimit_is_fatal) {
		...
		//当物理内存达到限制时，也会同步触发调用memorystatus_kill_process_sync方法，直接杀死对应的进程，如果pid为-1则杀死队列头部的进程。
...
//此方法直接(同步)杀死对应的进程，如果pid为-1则杀死队列头部比当前优先级低的进程;若在同一个优先级下面的话，会杀掉相对内存占用大的进程.
static boolean_t
...
memorystatus_kill_hiwat_proc(uint32_t *errors, boolean_t *purged, uint64_t *memory_reclaimed)
{
	...
	while (next_p) {
		...
		//继续寻找下一个优先级次低的进程,直到找到内存超过阈值的进程,将通过 memorystatus_do_kill 杀掉这个进程，并结束循环。
		p = next_p;
		next_p = memorystatus_get_next_proc_locked(&i, p, TRUE);
		...
		skip = (footprint_in_bytes <= memlimit_in_bytes); //如果这个进程内存小于阈值,就skip
		...
		if (skip) {
			...
			//调用 memorystatus_kill_proc 杀掉进程p
...
//此方法异步触发回收进程，调用memorystatus_thread_wake 唤醒memorystatus_thread。
static boolean_t
memorystatus_kill_process_async(pid_t victim_pid, uint32_t cause)
{
	...
	/*
	通过设置一个kill_under_pressure_cause 标记位，
	然后通过memorystatus_thread_wake 唤醒专门管理内存状态的线程去触发OOM
	*/
...
#if CONFIG_JETSAM
//由脏内存压缩造成抖动触发JetSAM回收进程boolean_t
...
//由内存映射文件被频繁读写造成文件缓存抖动，触发JetSAM回收进程
boolean_t
memorystatus_kill_on_FC_thrashing(boolean_t async)
{
	if (async) {
		//异步杀，通过唤醒 JetSAM thread走 OOM检测流程，也就是 memorystatus_thread内存管理流程
