...
//将obj存到指定地址location
//__strong类型的变量的作用域结束时 或赋值之前会调用此函数。如果是变量赋值，就先retain obj对象，再释放location原来指向的对象；如果变量作用域结束，就会将NULL存到*location，再释放location原来指向的对象；
void
objc_storeStrong(id *location, id obj)  //location:&a , obj:null
{
    id prev = *location;
    if (obj == prev) { //如果当前对象与目标对象相同，直接返回
        return;
    }
    objc_retain(obj);
    *location = obj;
...
storeWeak(id *location, objc_object *newObj)
{
    ...
    id oldObj;
    SideTable *oldTable; //旧的引用弱表
    SideTable *newTable; //新的弱引用表
 ...
 retry:
    ...
    if (haveOld) { //将旧的weak指针地址移除
        weak_unregister_no_lock(&oldTable->weak_table, oldObj, location);
    }

    // Assign new value, if any.
    if (haveNew) { //将新的weak指针地址添加到弱引用表中
        newObj = (objc_object *)
        ...
        if (newObj  &&  !newObj->isTaggedPointer()) {
            newObj->setWeaklyReferenced_nolock();  //修改weak新引用的对象的bit标志位
        }
 ...
 *
 * @param location Address of __weak ptr.   //_weak指针的地址，有了它可以在最后将其指向的对象置为nil。
 ...
 */
//在weak对象超过作用域时，会调用objc_destroyWeak进行释放
void
...
// 可知AutoreleasePoolPage 是继承自AutoreleasePoolPageData
class AutoreleasePoolPage : private AutoreleasePoolPageData
{
	friend struct thread_data_t;

public:
    //内存按这个大小进行分配，可以看出autoreleasePool的大小为vm page size的倍数
	static size_t const SIZE =
#if PROTECT_AUTORELEASEPOOL
		PAGE_MAX_SIZE;  // must be multiple of vm page size
#else
		PAGE_MIN_SIZE;  // size and alignment, power of 2 ,大小和对齐也是按页
#endif
    
private:
	static pthread_key_t const key = AUTORELEASE_POOL_KEY;
	static uint8_t const SCRIBBLE = 0xA3;  // 0xA3A3A3A3 after releasing
	static size_t const COUNT = SIZE / sizeof(id);  //每次申请的这块内存里的 AutoreleasePoolPage对象数

    // EMPTY_POOL_PLACEHOLDER is stored in TLS when exactly one pool is 
    // pushed and it has never contained any objects. This saves memory 
    // when the top level (i.e. libdispatch) pushes and pops pools but 
    // never uses them.
#   define EMPTY_POOL_PLACEHOLDER ((id*)1)  //在push时，保存在TLS中
...
#   define POOL_BOUNDARY nil
    ...
    //把obj对象添加到page当中，并返回obj对象的地址
    id *add(id obj)
    {
        ASSERT(!full());
        unprotect();
        id *ret = next;  // faster than `return next-1` because of aliasing
        *next++ = obj; //把obj放到next指向的位置后，next++向后挪动1个obj大小的距离
        protect();
        return ret;
    }
    ...
    static inline id *autoreleaseFast(id obj)
    {
        ...
        if (page && !page->full()) { //当前 page 不满
            //将obj对象添加至 AutoreleasePoolPage
            return page->add(obj);
        } else if (page) { //当前 page 已满
            // 初始化一个新的页,并把obj加入
            return autoreleaseFullPage(obj, page);
        } else {
            //无 hotPage,就创建一个 hotPage，并把obj加入
    ...
    id *autoreleaseFullPage(id obj, AutoreleasePoolPage *page)
    {
        ...
        //遍历查找一个未满的AutoreleasePoolPage
        do {
            if (page->child) page = page->child;
            //如果没找到，就创建一个新的 AutoreleasePoolPage
            else page = new AutoreleasePoolPage(page);
        } while (page->full());

        setHotPage(page);  //设置为当前页
        return page->add(obj); //把obj加入当前页
    }
    ...
    id *autoreleaseNoPage(id obj)
    {
        ...
        //当前内存中不存在 AutoreleasePoolPage，就要从头开始构建这个自动释放池的双向链表
...
/**
__attribute__是一个编译器指令，用于指定声明的特征，从而可以进行更多的错误检查和高级优化。
该关键字的语法是__attribute__，后跟两组括号（双括号使“宏”变得容易“删除”，尤其是具有多个属性的情况）。 括号内是逗号分隔的属性列表。 __attribute__指令放在函数，变量和类型声明之后。
*/
...
/*C语言里的objc_release会调用C++的objc_object::release inline函数，C++的 inline函数会调用OC NSObject的release函数，在OC的release中会调用C的_objc_rootRelease，最后在_objc_rootRelease里是通过调用C++的objc_object::rootRelease实现的。
*/
__attribute__((aligned(16), flatten, noinline))
void 
objc_release(id obj)
...
/*
 最终是通过C++的AutoreleasePoolPage::autorelease实现
 */
__attribute__((aligned(16), flatten, noinline))
id
...
callAlloc(Class cls, bool checkNil, bool allocWithZone=false)
{
    ...
    //判断当前class是否有自定义的allocWithZone
    if (fastpath(!cls->ISA()->hasCustomAWZ())) {
        return _objc_rootAllocWithZone(cls, nil);
    }
#endif

    // No shortcuts available.  无编译器优化
    if (allocWithZone) {
...
//__autoreleasing声明的变量,在赋值之前，会调用这个函数
id
...
//new 就是组合了对 callAlloc 与 init 的调用
+ (id)new {
...
//返回了强转的self,这是构造方法也是工厂设计，方便我们重写init方法
- (id)init {
