...
static int loadable_classes_used = 0; //列表的使用量
static int loadable_classes_allocated = 0; //列表的容量
...
void add_class_to_loadable_list(Class cls)
{
    ...
    //如果这个类没有load方法就直接返回了，也不会把这个类添加到全局的load列表里
...
static bool call_category_loads(void)
{
    ...
    used -= shift; //减去used里的空位，就是实际满的位置
...
void call_load_methods(void)
{
    ...
    //将所有类与分类的load方法会放到autoreleasePool中去调用
