...
//CacheLookup的宏定义代码,得知缓存中寻找的三种形式：
//2 CacheHit：找到直接返回
//3 Miss：找不到的话直接
//1 Scan More：在其它地方找到的话通过汇编直接add进缓存中
						// do {
1:	ldp	p17, p9, [x13], #-BUCKET_SIZE	//     {imp, sel} = *bucket--
	cmp	p9, p1				//     if (sel != _cmd) {
	b.ne	3f				//         scan more ,在其他地方找
						//     } else {
2:	CacheHit \Mode				// hit:    call or return imp 找到直接返回
						//     }
...
//objc_msgSend通过对象的isa指针获得类的结构体，先在Cache里找，找到就执行，没找到就在分发列表里查找方法的selector，没找到就通过objc_msgSend结构体中指向父类的指针找到父类，然后在父类分发列表找，直到root class（NSObject）。
...
//主要函数,在缓存列表中找imp,这里CacheLookup有三种方式：NORMAL|GETIMP|LOOKUP
// 1、成功：call imp；2、失败：objc_msgSend_uncached；
...
.endmacro
	...
	MethodTableLookup //这个宏里调用 lookupIMPOrForward
