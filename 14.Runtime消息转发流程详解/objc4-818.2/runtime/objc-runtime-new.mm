...
/*
 isFixedUp为runtime维护的一个变量fixed_up_method_list，通过prepareMethodLists来更新，而这个方法在attachCategories阶段和addMethod完成后会调用
 */
...
load_images(const char *path __unused, const struct mach_header *mh)
{
    ...
    // Discover load methods
    {
        ...
        prepare_load_methods((const headerType *)mh); //从mach-o文件加载所有的类
    }
...
static void schedule_class_load(Class cls)
{
    ...
    //设置class_rw_t结构里的读写标志设置为RW_LOADED
...
void prepare_load_methods(const headerType *mhdr)
{
    ...
    //从mach-o文件中的__objc_nlclslist段中把非懒加载的类读取出来
    classref_t const *classlist = 
        _getObjc2NonlazyClassList(mhdr, &count);
    
    //映射ClassList中所有的类，并加载
    for (i = 0; i < count; i++) {
        schedule_class_load(remapClass(classlist[i]));
    }

    //从mach-o文件中的___objc_nlcatlist段中把不是懒加载的分类读取出来
...
static void resolveInstanceMethod(id inst, SEL sel, Class cls)
{
    ...
    //判断是否实现了 resolveInstanceMethod: ，如果没实现直接返回。
    if (!lookUpImpOrNilTryCache(cls, resolve_sel, cls->ISA(/*authenticated*/true))) {
        // Resolver not implemented.
        return;
    }

    //调用resolveInstanceMethod，获得返回值resolved
    BOOL (*msg)(Class, SEL, SEL) = (typeof(msg))objc_msgSend;
    bool resolved = msg(cls, resolve_sel, sel);

    // Cache the result (good or bad) so the resolver doesn't fire next time.
    // +resolveInstanceMethod adds to self a.k.a. cls
    //重新进行lookUpImpOrNil并禁用resolver
    IMP imp = lookUpImpOrNilTryCache(inst, sel, cls);
...
//lookUpImpOrNil不会返回含有forward类型的IMP，也就是说不会触发消息转发。
...
/*
 lookUpImpOrForward实现了一套查找 IMP 的标准路径，也就是在消息转发（Forward）之前的逻辑，此方法只负责返回 IMP，没有直接参与消息的转发。
 在lookUpImpOrForward环节中，如果没有进行转发，那么最终会返回_objc_msgForward_impcache,在arm64的汇编中找到该函数的实现。
*/
NEVER_INLINE
IMP lookUpImpOrForward(id inst, SEL sel, Class cls, int behavior)
{
    //获得一个转发类型的imp，用const修饰不可被修改
...
addMethod(Class cls, SEL name, IMP imp, const char *types, bool replace)
{
    ...
    method_t *m;
    //先通过调用getMethodNoSuper_nolock去类的方法列表中查找
    if ((m = getMethodNoSuper_nolock(cls, name))) { //找到就判断传进来的replace
        // already exists
        if (!replace) {
            result = m->imp(false);
        } else {
        ...
        // fixme optimize
        //创建一个method_list_t,把方法信息赋值到method_list_t中
