...
struct __CFRunLoopMode {
    ...
    Boolean _stopped;
    char _padding[3];
    CFMutableSetRef _sources0;   //Source0 : 触摸事件，PerformSelectors
    CFMutableSetRef _sources1;   //source1:基于Port的线程间通信
...
struct __CFRunLoop {
    ...
    CFRunLoopModeRef _currentMode; //CFRunLoopModeRef 其实是指向__CFRunLoopMode结构体的指针
...
static Boolean __CFRunLoopServiceMachPort(mach_port_name_t port, mach_msg_header_t **buffer, size_t buffer_size, mach_port_t *livePort, mach_msg_timeout_t timeout, voucher_mach_msg_state_t *voucherState, voucher_t *voucherCopy) {
    ...
    for (;;) {		/* In that sleep of death what nightmares may come ... */
        ...
        //mach_msg -> mach_msg_trap() //让当前线程陷入休眠
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
	...
	__CFRunLoopDoBlocks(rl, rlm);
        ...
        if (sourceHandledThisLoop) {  // 如果处理Source0的结果是rrue
            // 处理Blocks
            __CFRunLoopDoBlocks(rl, rlm);
	}
