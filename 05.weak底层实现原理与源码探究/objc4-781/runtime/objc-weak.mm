...
static void append_referrer(weak_entry_t *entry, objc_object **new_referrer)
{
    if (! entry->out_of_line()) {
        ...
        // 如果inline_referrers的位置已经存满了，则要转型为referrers，分配并使用动态数组
    ...
    if (entry->num_refs >= TABLE_SIZE(entry) * 3/4) {
        return grow_refs_and_insert(entry, new_referrer); // 扩容，并插入对象
    }
    // 如果不需要扩容，直接插入到weak_entry中。注意，weak_entry是一个哈希表，key：w_hash_pointer(new_referrer) value: new_referrer；
    ...
    while (entry->referrers[index] != nil) {
        ...
        //这里entry->mask取值一定是如：0x111, 0x1111, 0x11111 ... ，因为数组每次都是*2增长，即8， 16， 32，对应动态数组空间长度-1的mask，也就是前面的取值。
        index = (index+1) & entry->mask;
        if (index == begin) bad_weak_table(entry);  // index == begin 说明在referrers数组没有找到合适位置
    }
    
    // 记录最大的hash冲突次数, max_hash_displacement指示: 尝试至多max_hash_displacement次，肯定能够找到object对应的hash位置
    if (hash_displacement > entry->max_hash_displacement) {
        entry->max_hash_displacement = hash_displacement;
    }
    
    // 将new_referrer存入动态数组，同时，更新元素个数num_refs
...
weak_entry_for_referent(weak_table_t *weak_table, objc_object *referent)
{
    ...
    size_t begin = hash_pointer(referent) & weak_table->mask; // 通过 & weak_table->mask的位操作，来确保index不会越界
    size_t index = begin;
    size_t hash_displacement = 0;
                        ...
                        id *referrer_id)
{
    ...
    if ((entry = weak_entry_for_referent(weak_table, referent))) {
        ...
        // 移除元素之后， 检查weak_entry_t的hash数组是否已经空了
        bool empty = true;
 ...
 * 
 * @param weak_table The global weak table.  //weak_table_t 结构类型的全局的弱引用表
...
{
    objc_object *referent = (objc_object *)referent_id; //weak指针的对象 newObj
    ...
    bool deallocating;
    if (!referent->ISA()->hasCustomRR()) { //确保被引用的对象可用（没有在析构，同时应该支持weak引用）
...
weak_clear_no_lock(weak_table_t *weak_table, id referent_id) 
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (referrer) {
            // 如果weak ptr就是referent引用的weak对象地址，则将weak ptr设置为nil，这就是为什么weak 指针会自动设置为nil的原因
            if (*referrer == referent) {
                *referrer = nil;
            }
            // 如果weak ptr不空且不是referent引用的weak对象地址，这可能是由于runtime代码的逻辑错误引起的，报错
            else if (*referrer) {
